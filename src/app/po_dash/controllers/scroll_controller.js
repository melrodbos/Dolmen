...
angular.module('dolmen.scroll', [] )
      ...
      compile : function(){
        ...
        return function(scope, element, attr) {
          element.bind("click", function(event){
            ScrollTo.features(attr.scrollTo, attr.offset);
          });
        };
      }
    };
  ...
  .service('ScrollTo', ['$window', 'ngScrollToOptions', function($window, ngScrollToOptions) {
    ...
    this.features = function (features, offset, focus) {//find element with the given id or name and scroll to the first element it finds
        var document = $window.document;

        if(!features) {//move to top if idOrName is not provided
          $window.scrollTo(0, 0);
        }
